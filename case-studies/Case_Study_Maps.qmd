...
# Display both the map and the table (useful for R Markdown or Shiny)
map
paged_table
...
# Create a paged table for unique titles
Dallas_unique_titles <- Dallas_data_unique %>%
  distinct(title) %>%
  arrange(title)

Dallas_paged_table <- datatable(
  Dallas_unique_titles,
  options = list(pageLength = 10),  # Number of rows per page
  rownames = FALSE,
...
# Create a paged table for unique titles
Portland_unique_titles <- Portland_data_unique %>%
  distinct(title) %>%
  arrange(title)

Portland_paged_table <- datatable(
  Portland_unique_titles,
  options = list(pageLength = 10),  # Number of rows per page
  rownames = FALSE,
...
# Create a paged table for unique titles
Detroit_unique_titles <- Detroit_data_unique %>%
  distinct(title) %>%
  arrange(title)

Detroit_paged_table <- datatable(
  Detroit_unique_titles,
  options = list(pageLength = 10),  # Number of rows per page
  rownames = FALSE,
  caption = "Detroit Locations"
)
